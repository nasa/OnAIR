...
class Agent:
    def __init__(
        self, vehicle, learners_plugin_dict, planners_plugin_dict, complex_plugin_dict
    ):
        ...
        # AI Interfaces
        self.learning_systems = LearnersInterface(
            self.vehicle_rep.get_headers(), learners_plugin_dict
        )
        self.planning_systems = PlannersInterface(
            self.vehicle_rep.get_headers(), planners_plugin_dict
        )
        self.complex_reasoning_systems = ComplexReasoningInterface(
            self.vehicle_rep.get_headers(), complex_plugin_dict
        )

    def reason(self, frame):
        aggregate_high_level_info = {}
        self.vehicle_rep.update(frame)
        aggregate_high_level_info["vehicle_rep"] = (
            self.vehicle_rep.get_state_information()
        )
        self.learning_systems.update(
            self.vehicle_rep.curr_data, aggregate_high_level_info
        )
        aggregate_high_level_info["learning_systems"] = (
            self.learning_systems.render_reasoning()
        )
        self.planning_systems.update(aggregate_high_level_info)
        aggregate_high_level_info["planning_systems"] = (
            self.planning_systems.render_reasoning()
        )

        return self.complex_reasoning_systems.update_and_render_reasoning(
            aggregate_high_level_info
        )
