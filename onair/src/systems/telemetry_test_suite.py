...
class TelemetryTestSuite:
    ...
    def run_tests(self, header_index, test_val, sync_data):
        ...
        for test in tests:
            ...
            stat, mass_assignments = self.all_tests[test_name](
                test_val, test_data, self.epsilon
            )
    ...
    def feasibility(self, val, test_params, epsilon):
        ...
        deltas = [
            abs(test_params[i + 1] - test_params[i])
            for i in range(0, len(test_params) - 1)
        ]
    ...
    def get_status_specific_mnemonics(self, status="RED"):
        names = [
            res.get_name() for res in self.latest_results if res.get_status() == status
        ]
        return names
