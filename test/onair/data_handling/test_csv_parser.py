...
def test_CSV_process_data_file_sets_sim_data_to_parse_csv_data_return_and_frame_index_to_zero(
    ...
    mocker.patch.object(
        pytest.cut, "parse_csv_data", return_value=forced_return_parse_csv_data
    )
...
def test_CSV_parse_csv_data_returns_empty_list_when_parsed_dataset_is_empty(
    mocker, setup_teardown
):
    # Arrange
...
def test_CSV_parse_csv_data_returns_list_of_row_values_when_parsed_dataset(
    mocker, setup_teardown
):
    # Arrange
    arg_dataFile = MagicMock()
    ...
    for i in range(num_fake_rows):
        ...
        for j in range(pytest.gen.randint(1, 10)):  # arbitrary, from 1 to 10 row values
            fake_row_values.append(
                pytest.gen.randint(1, 10)
            )  # arbitrary, from 1 to 10 as a value in row
    ...
    mocker.patch(csv_parser.__name__ + ".csv.reader", return_value=fake_dataset)
    mocker.patch(
        csv_parser.__name__ + ".floatify_input", side_effect=expected_result_list
    )
...
def test_CSV_parse_meta_data_file_returns_call_to_extract_meta_data_handle_ss_breakdown(
    ...
    mocker.patch(
        csv_parser.__name__ + ".extract_meta_data_handle_ss_breakdown",
    ...
    assert csv_parser.extract_meta_data_handle_ss_breakdown.call_args_list[0].args == (
        arg_configFile,
        arg_ss_breakdown,
    )
...
def test_CSV_get_vehicle_metadata_returns_list_of_headers_and_list_of_test_assignments(
    setup_teardown,
):
    # Arrange
...
# CSV get_next test
def test_CSV_get_next_increments_index_and_returns_current_frame_of_data(
    setup_teardown,
):
    # Arrange
    fake_frame_index = 10
...
# CSV has_more test
def test_CSV_has_more_returns_true_when_index_less_than_number_of_frames(
    setup_teardown,
):
    # Arrange
    fake_frame_index = 10
...
def test_CSV_has_more_returns_false_when_index_equal_than_number_of_frames(
    setup_teardown,
):
    # Arrange
    fake_frame_index = 10
