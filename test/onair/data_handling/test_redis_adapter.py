...
# __init__ tests
def test_redis_adapter_DataSource__init__sets_redis_values_then_connects_and_subscribes_to_subscriptions(
    mocker,
):
    # Arrange
...
def test_redis_adapter_DataSource_subscribe_subscribes_to_each_given_subscription_and_starts_listening_when_server_available(
    ...
    for i in range(len(arg_subscriptions)):
        ...
        assert redis_adapter.print_msg.call_args_list[i].args == (
            f"Subscribing to channel: {arg_subscriptions[i]}",
        )
    assert threading.Thread.call_count == 1
    assert threading.Thread.call_args_list[0].kwargs == (
        {"target": cut.message_listener}
    )
...
def test_redis_adapter_DataSource_subscribe_states_no_subscriptions_given_when_server_does_not_respond_to_ping(
    mocker,
):
    # Arrange
...
def test_redis_adapter_DataSource_message_listener_warns_user_when_processed_data_did_not_contain_time(
    ...
    cut.currentData = {0: {"headers": [], "data": []}, 1: {"headers": [], "data": []}}
    cut.pubsub = MagicMock()
    cut.new_data_lock = MagicMock()
    cut.new_data = False
...
def test_redis_adapter_DataSource_parse_meta_data_file_returns_call_to_extract_meta_data_handle_ss_breakdown_and_sets_subscriptions_when_redis_subscriptions_occupied(
    mocker,
):
    # Arrange
    cut = DataSource.__new__(DataSource)
...
def test_redis_adapter_DataSource_parse_meta_data_file_returns_call_to_extract_meta_data_handle_ss_breakdown_and_sets_subscriptions_to_empty_when_none_given(
    mocker,
):
    # Arrange
    cut = DataSource.__new__(DataSource)
