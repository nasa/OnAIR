...
# connect tests
def test_sbn_adapter_DataSource_connect_starts_listener_thread_and_subscribes_to_messages(
    mocker,
):
    # Arrange
    cut = DataSource.__new__(DataSource)
    ...
    mocker.patch(
        sbn_adapter.__name__ + ".threading.Thread", return_value=fake_listener_thread
    )
...
def test_sbn_adapter_DataSource_gather_field_names_returns_field_name_if_type_not_defined_in_message_headers_and_no_subfields_available(
    mocker,
):
    # Arrange
    cut = DataSource.__new__(DataSource)
...
def test_sbn_adapter_Data_Source_gather_field_names_returns_nested_list_for_nested_structure(
    mocker,
):
    # Arrange
    cut = DataSource.__new__(DataSource)
    ...
    parent_field_type._fields_ = [
        (child1_field_name, child1_field_type),
    ...
    assert set(result) == set(
        [
            parent_field_name + "." + child2_field_name,
...
def test_sbn_adapter_DataSource_parse_meta_data_file_calls_rasies_ConfigKeyError_when_channels_not_in_config(
    mocker,
):
    # Arrange
    cut = DataSource.__new__(DataSource)
...
def test_sbn_adapter_DataSource_parse_meta_data_file_populates_lookup_table_and_current_data_on_ideal_config(
    mocker,
):
    # Arrange
    cut = DataSource.__new__(DataSource)
    ...
    mocker.patch(
        sbn_adapter.__name__ + ".extract_meta_data_handle_ss_breakdown",
    ...
    # Assert
    assert cut.msgID_lookup_table == {
        1: ["AppName1", mock_struct_1],
        2: ["AppName2", mock_struct_2],
    }
...
def test_sbn_adapter_Data_Source_get_current_data_calls_gather_field_names_correctly(
    mocker,
):
    # Arrange
    cut = DataSource.__new__(DataSource)
...
def test_sbn_adapter_DataSource_get_current_data_unpacks_sub_fields_correctly(mocker):
    ...
    cut.__setattr__(
        "currentData", [{"headers": [], "data": []}, {"headers": [], "data": []}]
    )
    for x in range(0, 2):
    ...
    expected_data = {
        "headers": [
            arg_app_name + "." + "field1.temperature",
