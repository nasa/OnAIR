...
def test_LearnersInterface__init__sets_self_headers_to_given_headers_and_sets_self_learner_constructs_to_return_value_of_import_plugins(
    mocker,
):
    # Arrange
    arg_headers = MagicMock()
    ...
    mocker.patch(
        learners_interface.__name__ + ".import_plugins",
        return_value=forced_return_learner_constructs,
    )
    ...
    assert learners_interface.import_plugins.call_args_list[0].args == (
        arg_headers,
        arg__learner_plugins,
    )
...
def test_LearnersInterface_update_calls_update_with_given_low_level_and_high_level_data_on_each_learner_constructs_item(
    mocker,
):
    # Arrange
    arg_low_level_data = MagicMock()
    ...
    num_fake_learner_constructs = pytest.gen.randint(
        1, 10
    ...
    for i in range(num_fake_learner_constructs):
        ...
        assert cut.learner_constructs[i].update.call_args_list[0].args == (
            arg_low_level_data,
            arg_high_level_data,
        )
...
def test_LearnersInterface_render_reasoning_returns_dict_of_each_ai_construct_as_key_to_the_result_of_its_render_reasoning_when_instance_learner_constructs_is_occupied(
    mocker,
):
    # Arrange
    cut = LearnersInterface.__new__(LearnersInterface)
