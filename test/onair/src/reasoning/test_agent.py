...
# __init__ tests
def test_Agent__init__sets_vehicle_rep_to_given_vehicle_and_learning_systems_and_mission_status_and_bayesian_status(
    mocker,
):
    # Arrange
    ...
    mocker.patch.object(
        arg_vehicle, "get_bayesian_status", return_value=fake_bayesian_status
    )
    mocker.patch(
        agent.__name__ + ".LearnersInterface", return_value=fake_learning_systems
    )
    mocker.patch(
        agent.__name__ + ".PlannersInterface", return_value=fake_planning_systems
    )
    mocker.patch(
        agent.__name__ + ".ComplexReasoningInterface", return_value=fake_complex_systems
    )
    ...
    cut.__init__(
        arg_vehicle,
        arg_learners_plugin_dict,
        arg_planners_plugin_dict,
        arg_complex_plugin_dict,
    )
...
# reason tests
def test_Agent_reason_updates_vehicle_rep_with_given_frame_learners_with_frame_and_aggregated_high_level_data_planners_with_aggreagated_high_level_data_returning_complex_reasonings_update_and_render_reasoning(
    mocker,
):
    # Arrange
    arg_frame = MagicMock()
    ...
    expected_aggregate_to_planners = {
        "vehicle_rep": fake_vehicle_rep_state,
        "learning_systems": fake_learning_systems_reasoning,
    }
    expected_aggregate_to_complex = {
        "vehicle_rep": fake_vehicle_rep_state,
        "learning_systems": fake_learning_systems_reasoning,
    ...
    mock_manager.attach_mock(
        mocker.patch.object(fake_vehicle_rep, "update"), "cut.vehicle_rep.update"
    )
    mock_manager.attach_mock(
        mocker.patch.object(
            fake_vehicle_rep,
            "get_state_information",
            return_value=fake_vehicle_rep_state,
        ),
        "cut.vehicle_rep.get_state_information",
    )
    mock_manager.attach_mock(
        mocker.patch.object(fake_learning_systems, "update"),
        "cut.learning_systems.update",
    )
    mock_manager.attach_mock(
        mocker.patch.object(
            fake_learning_systems,
            "render_reasoning",
            return_value=fake_learning_systems_reasoning,
        ),
        "cut.learning_systems.render_reasoning",
    )
    mock_manager.attach_mock(
        mocker.patch.object(fake_planning_systems, "update"),
        "cut.planning_systems.update",
    )
    mock_manager.attach_mock(
        mocker.patch.object(
            fake_planning_systems,
            "render_reasoning",
            return_value=fake_planning_systems_reasoning,
        ),
        "cut.planning_systems.render_reasoning",
    )
    mock_manager.attach_mock(
        mocker.patch.object(
            fake_complex_reasoning_systems,
            "update_and_render_reasoning",
            return_value=expected_result,
        ),
        "cut.complex_reasoning_systems.update_and_render_reasoning",
    )
    ...
    mock_manager.assert_has_calls(
        [
            ...
            mocker.call.cut.learning_systems.update(
                fake_vehicle_rep.curr_data, expected_aggregate_to_complex
            ),
            ...
            mocker.call.cut.complex_reasoning_systems.update_and_render_reasoning(
                expected_aggregate_to_complex
            ),
        ],
        any_order=False,
