...
def test_ExecutionEngine__init__sets_expected_values_but_does_no_calls_when_config_file_is_empty_string(
    mocker,
):
    # Arrange
    arg_config_file = ""
...
def test_ExecutionEngine__init__does_calls_when_config_file_is_an_occupied_string(
    mocker,
):
    # Arrange
...
def test_ExecutionEngine__init__accepts_no_arguments_using_defaults_instead_with_config_file_default_as_empty_string(
    mocker,
):
    # Arrange
    cut = ExecutionEngine.__new__(ExecutionEngine)
...
def test_ExecutionEngine_parse_configs_raises_FileNotFoundError_when_config_cannot_be_read(
    mocker,
):
    # Arrange
    ...
    # Assert
    assert e_info.match(
        f"Config file at '{arg_bad_config_filepath}' could not be read."
    )


def test_ExecutionEngine_parse_configs_raises_KeyError_with_config_file_info_when_the_required_key_FILES_is_not_in_config(
    mocker,
):
    # Arrange
...
def test_ExecutionEngine_parse_configs_raises_KeyError_with_config_file_info_when_a_required_FILES_subkey_is_not_in_config(
    ...
    fake_data_handling = {"DataSourceFile": MagicMock()}
    fake_plugins = {
...
def test_ExecutionEngine_parse_configs_skips_OPTIONS_when_the_required_section_OPTIONS_is_not_in_config(
    ...
    fake_plugins = [
        fake_knowledge_rep_plugin_list,
...
def test_ExecutionEngine_parse_configs_sets_all_items_without_error(mocker):
    ...
    mocker.patch.object(cut, "parse_plugins_dict", side_effect=fake_plugins)
    mocker.patch.object(fake_options, "getboolean", return_value=fake_IO_enabled)
    mocker.patch(execution_engine.__name__ + ".isinstance", return_value=True)
    mocker.patch(execution_engine.__name__ + ".os.path.exists", return_value=True)
    mocker.patch.object(fake_plugin_dict, "keys", return_value=fake_keys)
    mocker.patch.object(fake_plugin_dict, "__getitem__", return_value=fake_path)
...
def test_ExecutionEngine_parse_plugins_list_raises_ValueError_when_config_plugin_dict_is_not_dict(
    ...
    # Assert
    assert e_info.match(
        f"Plugin dict {arg_config_plugin_dict} from {fake_config_filepath} is invalid. It must be a dict."
    )
    ...
    assert execution_engine.isinstance.call_args_list[0].args == (
        fake_plugin_dict.body,
        execution_engine.ast.Dict,
    )


def test_ExecutionEngine_parse_plugins_list_raises_FileNotFoundError_when_single_config_plugin_dict_key_maps_to_non_existing_file(
    mocker,
):
    # Arrange
    arg_config_plugin_dict = MagicMock()
    ...
    # Assert
    assert e_info.match(
        f"In config file '{fake_config_filepath}' Plugin path '{fake_path}' does not exist."
    )
    assert cut.ast_parse_eval.call_count == 1
    assert cut.ast_parse_eval.call_args_list[0].args == (arg_config_plugin_dict,)
    assert execution_engine.isinstance.call_count == 1
...
def test_ExecutionEngine_parse_plugins_list_raises_FileNotFoundError_when_any_config_plugin_dict_key_maps_to_non_existing_file(
    mocker,
):
    # Arrange
    arg_config_plugin_dict = MagicMock()
    ...
    mocker.patch.object(fake_temp_plugin_dict, "values", return_value=fake_values)
    mocker.patch(
        execution_engine.__name__ + ".os.path.exists", side_effect=exists_side_effects
    )
...
def test_ExecutionEngine_parse_services_list_raises_ValueError_when_config_service_dict_is_not_dict(
    ...
    mocker.patch(execution_engine.__name__ + ".isinstance", return_value=False)
    mocker.patch.object(
        arg_config_service_dict, "items", return_value=[(None, arg_config_service_dict)]
    )
...
def test_ExecutionEngine_parse_services_list_sets_service_without_error(
    ...
    mocker.patch(execution_engine.__name__ + ".isinstance", return_value=True)
    mocker.patch.object(
        arg_config_service_dict,
        "items",
        return_value=[(fake_service_name, arg_config_service_dict)],
    )
    mocker.patch(
        execution_engine.__name__ + ".ast.literal_eval",
        return_value=fake_temp_service_dict,
...
def test_ExecutionEngine_setup_services_sets_self_service_manager_to_new_ServiceManager(
    mocker,
):
    # Arrange
    cut = ExecutionEngine.__new__(ExecutionEngine)
    ...
    mocker.patch(
        execution_engine.__name__ + ".ServiceManager", return_value=fake_service_manager
    )
...
def test_ExecutionEngine_parse_data_sets_the_simDataSource_to_a_new_data_source_module_DataSource(
    mocker,
):
    # Arrange
    ...
    mocker.patch(
        execution_engine.__name__ + ".importlib.util.spec_from_file_location",
        return_value=fake_spec,
    )
    mocker.patch(
        execution_engine.__name__ + ".importlib.util.module_from_spec",
        return_value=fake_module,
    )
    mocker.patch.object(fake_spec, "loader.exec_module", return_value=None)
    mocker.patch.object(
        fake_module, "DataSource", return_value=fake_parser_class_instance
    )

    # Act
    cut.parse_data(
        arg_parser_file_name, arg_dataFile, arg_metadataFile, arg_subsystems_breakdown
    )

    # Assert
    assert execution_engine.importlib.util.spec_from_file_location.call_count == 1
    assert execution_engine.importlib.util.spec_from_file_location.call_args_list[
        0
    ].args == (
        "data_source",
        arg_parser_file_name,
    )
    ...
    assert fake_module.DataSource.call_args_list[0].args == (
        arg_dataFile,
        arg_metadataFile,
        arg_subsystems_breakdown,
    )
    assert cut.simDataSource == fake_parser_class_instance


def test_ExecutionEngine_parse_data_argument_subsystems_breakdown_optional_default_is_False(
    mocker,
):
    # Arrange
    arg_parser_file_name = MagicMock()
...
def test_ExecutionEngine_setup_sim_sets_self_sim_to_new_Simulator(mocker):
    ...
    assert execution_engine.Simulator.call_args_list[0].args == (
        cut.simDataSource,
...
def test_ExecutionEngine_run_sim_runs_but_does_not_save_results_when_save_flag_is_False(
    mocker,
):
    # Arrange
    cut = ExecutionEngine.__new__(ExecutionEngine)
...
def test_ExecutionEngine_init_save_paths_makes_tmp_and_models_and_diagnosis_directories_and_adds_them_to_os_environ(
    mocker,
):
    # Arrange
    ...
    mocker.patch(
        execution_engine.__name__ + ".os.path.join",
    ...
    assert (
        execution_engine.os.environ["ONAIR_DIAGNOSIS_SAVE_PATH"]
        == fake_tmp_diagnosis_path
    )
...
def test_ExecutionEngine_delete_save_paths_does_nothing_when_save_path_has_no_tmp_dir(
    mocker,
):
    # Arrange
    fake_save_path = str(MagicMock())
    ...
    mocker.patch(execution_engine.__name__ + ".rmtree")
    ...
    assert execution_engine.rmtree.call_count == 0
...
def test_ExecutionEngine_delete_save_paths_removes_tmp_tree_when_it_exists(mocker):
    ...
    assert execution_engine.os.listdir.call_count == 1
    assert execution_engine.os.listdir.call_args_list[0].args == (fake_save_path,)
    assert execution_engine.rmtree.call_count == 1
    assert execution_engine.rmtree.call_args_list[0].args == (fake_save_path + "/tmp",)
    assert execution_engine.print.call_count == 0


def test_ExecutionEngine_delete_save_paths_prints_error_message_when_rmtree_raises_OSError(
    mocker,
):
    # Arrange
    fake_save_path = str(MagicMock())
    ...
    mocker.patch(execution_engine.__name__ + ".os.listdir", return_value=fake_dirs)
    mocker.patch(
        execution_engine.__name__ + ".rmtree",
        side_effect=OSError(
            # 0 to 10 arbitrary error value for errno
            pytest.gen.randint(0, 10),
            fake_error_message,
        ),
    )
    mocker.patch(execution_engine.__name__ + ".print")
...
def test_ExecutionEngine_save_results_creates_expected_save_path_and_copies_proper_tree_to_it(
    ...
    fake_environ = {
        "ONAIR_SAVE_PATH": fake_onair_save_path,
        "ONAIR_TMP_SAVE_PATH": fake_onair_tmp_save_path,
    }
    fake_save_path = (
        fake_onair_save_path + "saved/" + arg_save_name + "_" + fake_complete_time
    )
    ...
    mocker.patch(
        execution_engine.__name__ + ".strftime", return_value=fake_complete_time
    )
    mocker.patch.dict(execution_engine.__name__ + ".os.environ", fake_environ)
    mocker.patch(execution_engine.__name__ + ".os.makedirs")
    mocker.patch(execution_engine.__name__ + ".copytree")
    ...
    assert execution_engine.copytree.call_count == 1
    assert execution_engine.copytree.call_args_list[0].args == (
        fake_onair_tmp_save_path,
        fake_save_path,
    )
...
def test_ExecutionEngine_ast_parse_eval_returns_call_to_ast_parse_with_mode_eval(
    mocker,
):
    # Arrange
