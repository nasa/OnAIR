...
def test_file_io_parse_associations_raises_KeyError_when_loaded_data_does_not_have_keyword_children(
    mocker,
):
    # Arrange
...
def test_file_io_parse_associations_does_not_print_when_loaded_data_children_is_empty(
    mocker,
):
    # Arrange
    arg_filepath = MagicMock()
...
def test_file_io_parse_associations_raises_KeyError_when_loaded_data_child_missing_name(
    mocker,
):
    # Arrange
    arg_filepath = MagicMock()
...
def test_file_io_parse_associations_raises_KeyError_when_loaded_data_child_missing_connections(
    mocker,
):
    # Arrange
    arg_filepath = MagicMock()
...
def test_file_io_parse_associations_does_not_print_when_loaded_data_child_conections_are_empty(
    mocker,
):
    # Arrange
    arg_filepath = MagicMock()
...
def test_file_io_parse_associations_raises_KeyError_when_loaded_data_child_connections_missing_target(
    mocker,
):
    # Arrange
    arg_filepath = MagicMock()
...
def test_file_io_parse_associations_raises_KeyError_when_loaded_data_child_connections_missing_weight(
    mocker,
):
    # Arrange
    arg_filepath = MagicMock()
    ...
    fake_data = {}
    fake_data["children"] = [
        {"name": "I have a name!", "connections": [{"target": "I have a target!"}]}
    ]
...
def test_file_io_parse_associations_prints_associations_in_reverse_sort_by_weight_when_data_is_properly_formed(
    mocker,
):
    # Arrange
    arg_filepath = MagicMock()
    ...
    for i in range(total_num_connections):
        ...
        # add to random child
        child_index = pytest.gen.randrange(
            0, total_num_children
        )  # from 0 to total_num_children - 1
        ...
        fake_connections.insert(
            pytest.gen.randint(0, len(fake_connections)), fake_connection
        )
        expected_prints.append(
            f"{fake_child['name']} --> {fake_target}, {str(fake_weight)}"
        )
