...
def test_plugin_import_returns_single_item_list_when_given_module_dict_contains_one_key_value_pair_no_init_and_not_already_in_sys(
    mocker,
):
    # Arrange
    arg_headers = MagicMock()
    ...
    mocker.patch.object(fake_module_path, "endswith", return_value=False)
    mocker.patch(
        plugin_import.__name__ + ".os.path.basename", return_value=fake_mod_name
    )
    mocker.patch(plugin_import.__name__ + ".os.path.join", return_value=fake_full_path)
    mocker.patch(
        plugin_import.__name__ + ".importlib.util.spec_from_file_location",
        return_value=fake_spec,
    )
    mocker.patch(
        plugin_import.__name__ + ".importlib.util.module_from_spec",
    ...
    assert import_mock.call_args_list[0].args == (
        f"{fake_mod_name}.{fake_mod_name}_plugin",
    )
    assert import_mock.call_args_list[0].kwargs == (
        {"fromlist": [f"{fake_mod_name}_plugin"]}
    )
    ...
    assert plugin_import.importlib.util.spec_from_file_location.call_args_list[
        0
    ].args == (fake_mod_name, fake_full_path)
    assert plugin_import.importlib.util.module_from_spec.call_count == 1
    assert plugin_import.importlib.util.module_from_spec.call_args_list[0].args == (
        fake_spec,
    )
...
def test_plugin_import_returns_single_item_list_when_given_module_dict_contains_one_key_value_pair_has_init_and_not_already_in_sys(
    mocker,
):
    # Arrange
    arg_headers = MagicMock()
    ...
    assert plugin_import.os.path.basename.call_count == 1
    assert plugin_import.os.path.basename.call_args_list[0].args == (
        expected_true_path,
    )
    assert plugin_import.importlib.util.spec_from_file_location.call_count == 1
    assert plugin_import.importlib.util.spec_from_file_location.call_args_list[
...
def test_plugin_import_returns_single_item_list_when_given_module_dict_contains_one_key_value_pair_no_init_and_exists_in_sys(
    mocker,
):
    # Arrange
    arg_headers = MagicMock()
...
def test_plugin_import_returns_two_item_list_when_given_module_dict_contains_two_key_value_pairs_that_use_same_module(
    mocker,
):
    # Arrange
    arg_headers = MagicMock()
    ...
    fake_full_path = MagicMock()
    arg_module_dict = {
        fake_construct_name_1: fake_module_path,
    ...
    mocker.patch.object(
        fake_plugin,
        "Plugin",
        side_effect=[fake_Plugin_instance_1, fake_Plugin_instance_2],
    )
    ...
    assert import_mock.call_args_list[1].kwargs == (
        {"fromlist": [f"{fake_mod_name}_plugin"]}
    )
    # # Without the stop of import_mock any other fails will also cause INTERNALERROR
    mocker.stop(import_mock)
