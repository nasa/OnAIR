...
def test_Kalman__init__initializes_variables_to_expected_values_when_given_all_args_except_window_size(
    ...
    mocker.patch(
        kalman_plugin.__name__ + ".np.diag", return_value=forced_diag_return_value
    )
    mocker.patch(
        kalman_plugin.__name__ + ".np.array", return_value=forced_array_return_value
    )
...
def test_Kalman_update_mutates_frames_attribute_as_expected_when_both_frames_and_arg_frame_are_not_empty_and_len_arg_frame_greater_than_len_frames():
    ...
    len_arg_frame = pytest.gen.randint(
        6, 10
    )  # arbitrary int greater than max len of fake_frames, from 6 to 10
    arg_frame = [MagicMock()] * len_arg_frame
...
def test_Kalman_update_mutates_frames_attribute_as_expected_when_both_frames_and_arg_frame_are_not_empty_and_len_arg_frame_less_than_len_frames():
    # Arrange
    len_fake_frames = pytest.gen.randint(
        6, 10
    )  # arbitrary int greater than max len of arg_frame, from 6 to 10
    fake_frames = [[MagicMock()]] * len_fake_frames
    fake_window_size = pytest.gen.randint(1, 10)  # arbitrary, random int from 1 to 10
...
def test_Kalman_render_reasoning_returns_value_returned_by_frame_diagnosis_function(
    mocker,
):
    # Arrange
    fake_frames = MagicMock()
    ...
    mocker.patch.object(
        cut, "frame_diagnosis", return_value=forced_frame_diagnose_return
    )
...
def test_Kalman_mean_calculates_return_value_by_dividing_sum_by_len(mocker):
    ...
    forced_sum_return_value = pytest.gen.uniform(
        1.0, 10.0
    )  # arbitrary, random float from 1.0 to 10.0
    forced_len_return_value = pytest.gen.uniform(
        1.0, 10.0
    )  # arbitrary, random float from 1.0 to 10.0
...
# test residual
def test_Kalman_residual_calculates_return_value_by_finding_the_abs_difference_of_given_args(
    mocker,
):
    # Arrange
    arg_predicted = pytest.gen.uniform(
        -10.0, 10.0
    )  # arbitrary, random float from -10.0 to 10.0
    arg_actual = pytest.gen.uniform(
        -10.0, 10.0
    )  # arbitrary, random float from -10.0 to 10.0
...
# test std_dev
def test_Kalman_std_dev_calculates_return_value_by_using_np_std_function_on_arg_data(
    mocker,
):
    # Arrange
    arg_data = MagicMock()
    ...
    mocker.patch(
        kalman_plugin.__name__ + ".np.std", return_value=forced_std_return_value
    )
...
# test predict
def test_Kalman_predict_smoothes_data_and_predicts_result_using_KalmanFilter_functions_as_expected_when_data_is_empty_and_initial_val_equals_None(
    mocker,
):
    # Arrange
    arg_data = []
...
def test_Kalman_predict_smoothes_data_and_predicts_result_using_KalmanFilter_functions_as_expected_when_data_is_empty_and_initial_val_is_not_None(
    mocker,
):
    # Arrange
    arg_data = []
    arg_forward_steps = MagicMock()
    arg_initial_val = pytest.gen.uniform(
        -10.0, 10.0
    )  # arbitrary, random float from -10.0 to 10.0
    ...
    assert fake_kf.smooth.call_count == 1
    assert fake_kf.smooth.call_args_list[0].args == (arg_data,)
    assert fake_kf.smooth.call_args_list[0].kwargs == {
        "initial_value": [arg_initial_val, 0]
    }
    assert fake_kf.predict.call_count == 1
    assert fake_kf.predict.call_args_list[0].args == (arg_data, arg_forward_steps)


def test_Kalman_predict_smoothes_data_and_predicts_result_using_KalmanFilter_functions_as_expected_when_initial_val_equals_None(
    mocker,
):
    # Arrange
...
def test_Kalman_predict_when_not_given_initial_val_arg_sets_initial_val_arg_equal_to_None(
    mocker,
):
    # Arrange
    len_arg_data = pytest.gen.randint(1, 10)  # arbitrary, random int from 1 to 10
...
def test_Kalman_predict_smoothes_data_and_predicts_result_using_KalmanFilter_functions_as_expected_when_initial_val_is_not_None(
    mocker,
):
    # Arrange
    len_arg_data = pytest.gen.randint(1, 10)  # arbitrary, random int from 1 to 10
...
def test_Kalman_predict_floatifies_args_and_smoothes_data_and_predicts_result_using_KalmanFilter_functions_as_expected_when_args_are_not_float_values(
    mocker,
):
    # Arrange
    len_arg_data = pytest.gen.randint(1, 10)  # arbitrary, random int from 1 to 10
    ...
    arg_forward_steps = MagicMock()
    arg_initial_val_float = pytest.gen.uniform(
        -10.0, 10.0
    )  # arbitrary, random float from -10.0 to 10.0
    ...
    assert fake_kf.smooth.call_args_list[0].kwargs == {
        "initial_value": [arg_initial_val_float, 0]
    }
    assert fake_kf.predict.call_count == 1
...
def test_Kalman_predictions_for_given_data_returns_expected_result_when_data_arg_has_only_one_element(
    mocker,
):
    # Arrange
    arg_data = [MagicMock()]
...
def test_Kalman_predictions_for_given_data_returns_expected_result_when_data_arg_has_more_than_one_element(
    mocker,
):
    # Arrange
    ...
    mocker.patch.object(
        forced_predict_return_value, "observations", forced_predict_return_value
    )
...
def test_Kalman_generate_residuals_for_given_data_raises_error_when_data_arg_is_empty(
    mocker,
):
    # Arrange
    arg_data = []
...
def test_Kalman_generate_residuals_for_given_data_returns_expected_result_when_data_arg_has_only_one_element(
    mocker,
):
    # Arrange
    arg_data = [MagicMock()]
...
def test_Kalman_generate_residuals_for_given_data_returns_expected_result_when_data_arg_has_more_than_one_element(
    mocker,
):
    # Arrange
    len_data = pytest.gen.randint(2, 10)  # arbitrary, random int from 1 to 10
...
def test_Kalman_current_attribute_chunk_get_error_returns_true_when_abs_of_mean_residuals_equal_to_or_greater_than_one_point_five(
    mocker,
):
    # Arrange
    arg_data = MagicMock()
    ...
    forced_abs_return_value = pytest.gen.uniform(
        1.5, 10.0
    )  # random float, greater than cutoff value 1.5

    mocker.patch.object(
        cut,
        "generate_residuals_for_given_data",
        return_value=forced_generate_residuals_return_value,
    )
...
def test_Kalman_current_attribute_chunk_get_error_returns_false_when_abs_of_mean_residuals_less_than_one_point_five(
    mocker,
):
    # Arrange
    arg_data = MagicMock()
    ...
    forced_abs_return_value = pytest.gen.uniform(
        0.0, 1.49
    )  # random float, less than cutoff value 1.5
...
def test_Kalman_frame_diagnosis_returns_empty_list_when_current_attribute_chunk_get_error_always_returns_false_and_args_not_empty(
    mocker,
):
    # Arrange
    ...
    mocker.patch.object(
        cut,
        "current_attribute_chunk_get_error",
        return_value=forced_get_error_return_value,
    )
    ...
    for i in range(len_args):
        assert cut.current_attribute_chunk_get_error.call_args_list[i].args == (
            arg_frame[i],
        )


def test_Kalman_frame_diagnosis_returns_empty_list_when_all_elements_in_headers_arg_match_time_str(
    mocker,
):
    # Arrange
    len_args = pytest.gen.randint(1, 10)  # arbitrary, random int from 1 to 10
...
def test_Kalman_frame_diagnosis_returns_expected_sublist_of_headers_when_headers_contains_strings_matching_time_str_and_the_result_of_current_attribute_chunk_get_error_is_not_constant(
    mocker,
):
    # Arrange
    len_args = pytest.gen.randint(1, 10)  # arbitrary, random int from 1 to 10
    ...
    mocker.patch.object(
        cut,
        "current_attribute_chunk_get_error",
        side_effect=forced_get_error_side_effect,
    )
