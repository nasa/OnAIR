...
# test init
def test_Kalman__init__initializes_variables_using_both_given_and_default_arguments_and_creates_filter_when_only_given_required_arguments(
    mocker,
):
    # Arrange
    arg_name = MagicMock()
    ...
    mocker.patch(
        kalman_plugin.__name__ + ".simdkalman.KalmanFilter", return_value=fake_kf
    )
    mocker.patch(
...
def test_Kalman__init__initializes_variables_using_given_arguments_and_creates_filter_when_given_required_and_optional_arguments_with_window_size_greater_than_2(
    mocker,
):
    # Arrange
    arg_name = MagicMock()
...
def test_Kalman_update_with_initially_empty_frames(mocker):
    ...
    arg_low_level_data = [
        pytest.gen.uniform(-10, 10) for _ in range(num_fake_data_points)
    ]
    mocker.patch(
        kalman_plugin.__name__ + ".floatify_input", return_value=arg_low_level_data
    )
...
def test_Kalman_update_with_existing_data_in_frames_but_less_than_full_window_size(
    mocker,
):
    # Arrange
    cut = Kalman.__new__(Kalman)
    ...
    cut.window_size = pytest.gen.randint(
        existing_data_points + 1, existing_data_points + 5
...
def test_Kalman_update_with_full_window_size(mocker):
    ...
    cut.frames = [
        frame.copy() for frame in original_frames
    ]  # Create a copy for the cut object
...
def test_Kalman_render_reasoning_returns_empty_list_when_all_values_below_threshold(
    mocker,
):
    # Arrange
    cut = Kalman.__new__(Kalman)
    ...
    cut.frames = [
        list(range(pytest.gen.randint(3, 7))) for _ in range(num_headers)
    ]  # Random frame sizes
    cut.headers = [f"header{i}" for i in range(num_headers)]
    cut.residual_threshold = pytest.gen.uniform(
        1.0, 5.0
    )  # Random threshold between 1.0 and 5.0

    # All residuals are below the threshold
    forced_residuals = np.array(
        [cut.residual_threshold - 0.1 - i * 0.1 for i in range(num_headers)]
    )
    ...
    assert (
        cut._generate_residuals.call_args_list[0].args == ()
    )  # No arguments passed to _generate_residuals
...
def test_Kalman_render_reasoning_returns_values_above_threshold(mocker):
    ...
    forced_residuals = np.array(
        [cut.residual_threshold - 0.1] * num_headers
    )  # Initialize all below threshold
    for i in failing_indices:
        forced_residuals[i] = (
            cut.residual_threshold + 0.1 + i * 0.1
        )  # Set failing headers above threshold
    ...
    assert set(result) == set(
        expected_result
    )  # Check if the result contains all expected failing headers
    assert cut._generate_residuals.call_count == 1
    assert (
        cut._generate_residuals.call_args_list[0].args == ()
    )  # No arguments passed to _generate_residuals
...
# test _predict
def test_Kalman__predict_smoothes_data_and_predicts_result_using_KalmanFilter_functions_as_expected_when_initial_val_is_set(
    mocker,
):
    # Arrange
    arg_subframe = []
...
def test_Kalman__generate_residuals_with_frame_data_length_less_than_3(mocker):
    ...
    cut.frames = [
        [pytest.gen.uniform(-10, 10) for _ in range(short_frame_length)]
        for _ in range(num_headers)
    ]
...
def test_Kalman__generate_residuals_with_sufficient_frame_data_length(mocker):
    ...
    cut.frames = [
        [pytest.gen.uniform(-10, 10) for _ in range(frame_length)]
        for _ in range(num_headers)
    ]

    mock_predict_result = MagicMock()
    mock_predict_result.observations.mean = np.array(
        [[pytest.gen.uniform(-100, 100)] for _ in range(num_headers)]
    )
    mocker.patch.object(cut, "_predict", return_value=mock_predict_result)

    expected_residuals = np.abs(
        np.subtract(
            [frame[-1] for frame in cut.frames],
    ...
    np.testing.assert_array_almost_equal(
        cut._predict.call_args[0][2], expected_initial_value
    )
