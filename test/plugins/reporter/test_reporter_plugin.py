...
# test update
def test_Reporter_update_saves_given_args_and_only_outputs_update_when_not_verbose_mode(
    mocker,
):
    # Arrange
    ...
    assert reporter_plugin.print.call_args_list[0].args == (
        f"{cut.component_name}: UPDATE",
    )
    assert cut.low_level_data == arg_low_level_data
    assert cut.high_level_data == arg_high_level_data


def test_Reporter_update_saves_given_args_and_outputs_all_info_when_verbose_mode(
    mocker,
):
    # Arrange
    arg_low_level_data = MagicMock(name="arg_low_level_data")
    ...
    assert reporter_plugin.print.call_args_list[1].args == (
        f" : headers {cut.headers}",
    )
    assert reporter_plugin.print.call_args_list[2].args == (
        f" : low_level_data {arg_low_level_data.__class__} = '{arg_low_level_data}'",
    )
    assert reporter_plugin.print.call_args_list[3].args == (
        f" : high_level_data {arg_high_level_data.__class__} = '{arg_high_level_data}'",
    )
    assert cut.low_level_data == arg_low_level_data
    assert cut.high_level_data == arg_high_level_data
...
def test_Reporter_render_reasoning_only_outputs_render_reasoning_when_not_verbose_mode(
    mocker,
):
    # Arrange
    cut = Reporter.__new__(Reporter)
    ...
    assert reporter_plugin.print.call_count == 1
    assert reporter_plugin.print.call_args_list[0].args == (
        f"{cut.component_name}: RENDER_REASONING",
    )
...
def test_Reporter_render_reasoning_outputs_all_info_when_verbose_mode(mocker):
    ...
    assert reporter_plugin.print.call_args_list[1].args == (
        f" : My low_level_data is {fake_low_level_data}",
    )
    assert reporter_plugin.print.call_args_list[2].args == (
        f" : My high_level_data is {fake_high_level_data}",
    )
