...
def test_cleanup_clean_all_removes_provided_file_types_back_two_directories_from_file_location_when_given_run_path_is_empty_string(mocker):
  ...
  # Act
  src.util.cleanup.clean_all(arg_run_path)

  # Assert
  assert src.util.cleanup.os.path.realpath.call_count == 1
  assert src.util.cleanup.os.path.dirname.call_count == 1
  assert src.util.cleanup.os.chdir.call_count == 1
  assert src.util.cleanup.os.chdir.call_args_list[0].args == (fake_run_path + '/../../',)
